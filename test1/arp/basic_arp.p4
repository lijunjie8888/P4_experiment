...
control MyIngress(
    ...
    action send_arp_reply(macAddr_t macAddr, ip4Addr_t IPAddr) {
        ...
        hdr.ethernet.srcAddr = macAddr; // 设置ARP报文类型为回应类型        
        hdr.arp.oper         = ARP_OPER_REPLY; // 设置ARP返回内容        
        ...
        standard_metadata.egress_spec = standard_metadata.ingress_port; // 从入端口转发出去
    }
    ...
    apply {
         ...
         */        
        if(hdr.ethernet.etherType == TYPE_IPV4) { // IPv4类型存在则进行IPv4转发
            ipv4_lpm.apply();
        }        
        else if(hdr.ethernet.etherType == TYPE_ARP) { // 响应ARP匹配
            arp_ternary.apply();
        }
    }
}
