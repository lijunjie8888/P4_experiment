...
def writeTunnelRules(p4info_helper, ingress_sw, egress_sw, tunnel_id,
    ...
    table_entry = p4info_helper.buildTableEntry(# 实体的名字到ID数字转换的方法
        table_name="MyIngress.myTunnel_exact",
        match_fields={
            "hdr.myTunnel.dst_id": tunnel_id # 匹配tunnel id
        },
    ...
    for response in sw.ReadTableEntries():
        for entity in response.entities:
            ...
            for p in action.params:
                print(p4info_helper.get_action_param_name(action_name, p.param_id), end=' ') # 打印参数名
                print('%r' % p.value, end=' ')
            print()
            print(entry)
            print('-----')
